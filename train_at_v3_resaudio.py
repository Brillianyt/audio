...
class AudioTextModel(nn.Module):
    def __init__(self, embed_dim=256, unfreeze=2, text_encoder="phoneme_bigru"):
        super().__init__()
...
def train_v3(cfg, args):
    ...
    for ep in range(start_ep, cfg.epochs + 1):
        ...
        n_hard_ep = min(300000, len(hard_dedup) * 10)  # 10x oversample
        ...
        for e, q, y, txts, _ in loader:
            ...
            with torch.cuda.amp.autocast():
                ...
                loss = crit(match_logit * scale, y) + 0.1 * margin + 0.2 * F.mse_loss(et, ea)
        ...
        def ev(ld):
            ...
            for e, q, y, txts, id_ in ld:
                ...
                _, _, et = model(e, txts)
                eq = model.encoder(q)
                cs = (et * eq).sum(-1)  # cosine similarity [-1, 1]

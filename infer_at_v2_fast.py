"""Fast AT v2 inference — single zip open, batch processing."""
...
def predict(zip_path, csv_path, prefix):
    ...
    for i in range(0, total, bs):
        ...
        e_w, q_w, txts = [], [], []
        for bp in batch:
            data = z.read(f'wav/{bp["id"]}_enroll.wav')
            wav, sr = sf.read(io.BytesIO(data), dtype='float32', always_2d=False)
            if wav.ndim > 1: wav = wav.mean(1)
            if sr != 16000:
                import torchaudio
                wav = torchaudio.functional.resample(torch.from_numpy(wav).unsqueeze(0), sr, 16000).squeeze(0).numpy()
            e_w.append(torch.from_numpy(wav.astype('float32')[:int(1.5*16000)]).float())
            
            data = z.read(f'wav/{bp["id"]}_query.wav')
            wav, sr = sf.read(io.BytesIO(data), dtype='float32', always_2d=False)
            if wav.ndim > 1: wav = wav.mean(1)
            if sr != 16000:
                import torchaudio
                wav = torchaudio.functional.resample(torch.from_numpy(wav).unsqueeze(0), sr, 16000).squeeze(0).numpy()
            q_w.append(torch.from_numpy(wav.astype('float32')[:int(1.5*16000)]).float())

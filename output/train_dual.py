...
class AudioAudioModel(nn.Module):
    ...
    def forward(self, e, q):
        ...
        cos = (ea * qa).sum(-1)  # for monitoring only
...
def train_audio(cfg, args):
    ...
    for ep in range(start_ep, cfg.epochs + 1):
        ...
        for e, q, y, e_txts, q_txts, _ in loader:
            ...
            for t in all_txts:
                t = t.lower().strip()
                if t not in word_map: word_map[t] = len(word_map)
            ...
            # ── BCE: primary loss on comparison head ──
            bce = F.binary_cross_entropy_with_logits(logit, y)
            loss = loss + bce  # BCE as primary, SupCon as regularizer
            ...
            if cs_n % 50 == 0:
                print(f"  [ep{ep}] b{cs_n} loss={ls/n:.3f} bce={bce.item():.3f} "
            ...
            ls += loss.item(); n += 1
            del cos_sim, logit, ea, qa, all_emb, loss, _cp, _cn
        ...
        def ev(ld):
            ...
            for e, q, y, _, _, id_ in ld:
                ...
                ps.append(torch.sigmoid(logit).cpu().numpy())  # learnable scale from model
